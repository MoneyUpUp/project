<template>
	<div>
        ...
        <h3>예금 추천</h3>
        <div v-if="depositRecommendations.length" class="recommendations-grid">
            <BaseCard v-for="rec in depositRecommendations" :key="rec.id" class="recommendation-card">
                <img :src="getBankLogoById(rec.bank_code)" alt="은행 로고" class="bank-logo">
        ...
        <h3>적금 추천</h3>
        <div v-if="savingRecommendations.length" class="recommendations-grid">
            <BaseCard v-for="rec in savingRecommendations" :key="rec.id" class="recommendation-card">
                <img :src="getBankLogoById(rec.bank_code)" alt="은행 로고" class="bank-logo">
                <p class="product-name"><strong>상품명:</strong> {{ rec.name }}</p>
                <p><strong>은행:</strong> {{ rec.bank }}</p>
                <p><strong>기간:</strong> {{ rec.save_trm }}개월</p>
...
onMounted(async () => {
    try {
        const response = await axios.get('http://127.0.0.1:8000/ai/recommend/',{headers:{
